...
def load_and_prepare_data(data_path):
    try:
        ...
        st.error(f"Error: Movie metadata file not found at {data_path}. Please ensure it's there.")
        st.stop()
    ...
    movies_df = movies_df[
        ['id', 'title', 'genres', 'runtime', 'original_language', 'overview']]  # Keep necessary columns
    ...
    genre_dummies = pd.DataFrame(mlb.fit_transform(movies_df_indexed['genre_names']), columns=mlb.classes_,
    ...
    movies_df['lang_processed'] = movies_df['original_language'].apply(lambda x: x if x in top_langs else 'other')
    # Re-index before get_dummies
    lang_dummies = pd.get_dummies(movies_df.set_index('id')['lang_processed'], prefix='lang')

    # --- Combine Features ---
    # Ensure indices align using the indexed DataFrame
    feature_cols_df = pd.DataFrame(index=movies_df_indexed.index)  # Start with correct index
    ...
    movie_features_scaled_df = pd.DataFrame(movie_features_scaled, columns=feature_cols_df.columns,
                                            index=movie_ids_index)
    ...
    filtered_movies_df = movies_df[movies_df['id'].isin(valid_movie_ids)].drop_duplicates(subset=['id']).set_index('id')
    ...
    return movie_features_scaled_df, movie_id_to_title, title_to_movie_id, filtered_movies_df[
        ['title', 'overview', 'genre_names']]


# --- Recommendation Function --- (Same as before)
def recommend_for_new_user(liked_movie_ids, n, all_movie_features_scaled, id_to_title_map):
    valid_liked_ids = [mid for mid in liked_movie_ids if mid in all_movie_features_scaled.index]
    if not valid_liked_ids: return pd.DataFrame(
        columns=['movieId', 'title', 'similarity_score'])  # Empty if no valid IDs
    ...
    sim_scores = pd.Series(similarities.flatten(), index=all_movie_features_scaled.index)
    ...
    recommendations_df = pd.DataFrame(
        {'movieId': top_n_recommendations.index, 'similarity_score': top_n_recommendations.values})
...
movie_features_scaled_df, movie_id_to_title, title_to_movie_id, movies_display_df = load_and_prepare_data(DATA_PATH)

# Check if data loading failed
if movie_features_scaled_df is None:
    st.warning("Could not load movie data. Please check the file path and data integrity.")
    st.stop()
    ...
    if selected_title and selected_title != "":  # Check if a movie title is selected
        ...
        if movie_id:
            ...
            # Check if max limit reached
                ...
                st.session_state.movie_search_key = ""  # Set it back to the default empty string
        else:
            st.error(f"Could not find ID for title '{selected_title}'. This shouldn't normally happen.")
            st.session_state.movie_search_key = ""  # Also clear if error


# --- Search Bar ---
movie_titles_list = [""] + sorted([title for title in title_to_movie_id.keys() if
                                   title])  # Ensure titles are strings and not None/NaN, add empty first option
...
if not st.session_state.selected_movies:
    ...
    for i, (movie_id, movie_title) in enumerate(st.session_state.selected_movies):
        ...
        with cols[col_index]:
            ...
            with st.container(border=True):
                ...
                st.image(
                    PLACEHOLDER_IMAGE_URL + movie_title.replace(" ", "+")[:20] + '\\n' + movie_title[20:40],
                    caption=movie_title[:30] + ('...' if len(movie_title) > 30 else ''),  # Truncate long captions
                ...
                if st.button(f"Remove", key=f"remove_{movie_id}"):
    ...
    if movies_to_remove:
        for mid, mtitle in movies_to_remove:
            ...
            movie_tuple_to_remove = next((item for item in st.session_state.selected_movies if item[0] == mid), None)
...
num_selected = len(st.session_state.selected_movies)  # Recalculate after potential removals
...
if recommend_button_pressed and num_selected > 0:
    ...
    if not recommendations.empty and movies_display_df is not None:
        ...
        recommendations_display = recommendations.merge(
            movies_display_df[['title', 'overview', 'genre_names']],  # Select columns from the re-indexed df
            left_on='movieId',
        ...
        recommendations_display['genres'] = recommendations_display['genre_names'].apply(
            lambda x: ', '.join(x) if isinstance(x, list) and x else 'N/A'
        )

        st.dataframe(
            recommendations_display[['title_x', 'similarity_score', 'genres', 'overview']],
            column_config={
                "title_x": "Title",
